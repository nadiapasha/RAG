...
def load_documents(doc_paths):
    documents = SimpleDirectoryReader(doc_paths, required_exts = ['.pdf']).load_data()
    print(f'Loaded {len(documents)} documents')
    print(f'First document {documents[0]}')
    return documents


def load_embedding__model(model_name):
...
if __name__ == '__main__':
    ...
    embeddings = load_embedding__model(model_name = cfg.EMB_MODEL)
